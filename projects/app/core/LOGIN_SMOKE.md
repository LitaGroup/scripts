...
启动并关弹窗
  ...
  └─ 不在登录页 → 点底部「我的」
        ...
        └─ 未弹出（已登录「我的」）
              ...
              → 等自动回到首页 MainActivity 后 **立刻点「我的」**（坐标，不空等 tab）
              → 若仍未进登录：再 **重启 App**（terminate + activate，不卸载 / 不清数据）
              → 点「我的」→ LoginActivity → 走登录流程
登录完成 → 进入「我的」→ **整单通过**
```

> **判定**：最终进入已登录「我的」即通过。已登录时：点退出 → 回首页立刻点「我的」进登录（禁止卡在访客首页）；必要时才重启。不做 clearApp。Chrome/Facebook Custom Tab 会先 `back` 回 Lite。
...
| SM-LOGIN-06 | Facebook 完整登录 | P1 | 手工 | `android-lite/login-facebook.android.lite.test.ts` |
| SM-LOGIN-07 | Google 完整登录 | P1 | 手工 | `android-lite/login-google.android.lite.test.ts` |
...
| SM-LOGIN-14 | Google→Facebook→手机号串联 | P1 | — | `android-lite/login-all.android.lite.test.ts` |
...
- **脚本**：`projects/app/android-lite/login-facebook.android.lite.test.ts`
- **门控**：见上文「统一登录门控」
...
- **期望**：`mePage` 或数字 `user_no`
- **脚本**：`projects/app/android-lite/login-google.android.lite.test.ts`
...
- **脚本**：`projects/app/android-lite/login-all.android.lite.test.ts`
- **前置**：设备已登录 Google + Facebook；`config.app.json` 手机号账号可用
- **流程**（切换时默认**不重启 App**）：
  1. Google 登录 → 已登录「我的」
  2. 设置 → 退出 → 回首页后**立刻点「我的」**（坐标点击；Facebook 退出若停在 Settings 会 dumpsys 纠偏并重试退出）→ 登录页
  3. Facebook 登录 → 已登录「我的」
  4. 同上：退出 → 立刻点「我的」→ 登录页
  5. 手机号登录 → 已登录「我的」→ 整单通过
- **期望**：每段登录后 `mePage` / 数字 `user_no`；最终停在手机号登录后的「我的」
- **与单测差异**：单测门控（SM-LOGIN-02/06/07）退出后若仍进不了登录会 **重启 App**；本用例同样在退出卡住时允许重启兜底，禁止空等在设置页/访客首页
