...
- **可执行用例脚本**：统一放在 `../android-lite/`（含 Android Lite `*.android.lite.test.ts` 与 iOS Lita `*.ios.lita.test.ts`）
- **本目录**：不再放正式 `*.test.ts`（仅保留兼容 `login.android.lita.check.ts`）
...
| 平台 | 工程 | 密码登录 | 入口可见 |
|------|------|----------|----------|
| iOS | lita-ios-v2 (`but.lita.ios`) | `../android-lite/login-phone-password.ios.lita.test.ts` | `../android-lite/login-entries.ios.lita.test.ts` |
