...
测试环境（test）用例。biz=`didibus-v202609`，设计依据见 `../CASES.md`（技术设计 v1.5.0：双 LuckydrawModule `lucky-gift` 扣券抽礼物/道具 + `lucky-mileage` 免费抽里程；账户体系为 mod_account，账户标识 `DIDIBUS-MILEAGE`，余额/流水在 `mod_account_user` / `mod_account_user_record`，按 (biz, trans_no) 幂等）。
...
| 003-gift-send | 非活动礼物过滤、sender×2/receiver×0.5 发券（向下取整）、榜单计分=金币数×buff 倍率（含高倍率礼物验证）、送礼总榜+日榜+收礼总榜（Redis+落库）、orderNo 幂等（流水 trans_no=send_/recv_+orderNo 各 1 条）、期外消息无副作用 |
| 004-draw | 扣费（normal/flying，lucky-gift 统一扣券）、里程必得（分 pool 档位动态读取）、双抽奖记录（lucky-gift/lucky-mileage 各一批，result 接口可查）、bus 推进、探索点发奖账本、探索不影响榜单（buff 在赠送环节计入）、轮播最新条 |
...
| 007-rank-query | 送礼/收礼总榜排序与 selfRank、日榜 Top1（round-top）、大区隔离、/detail 聚合五块（account/bus/luckyGift/dailyTop1/marquee） |
