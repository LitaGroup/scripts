...
## Layout
- Scripts: `projects/<category>/<activity>/{checks,tests,exports}` (current: `projects/active/pk/{checks,tests,exports}`, `projects/funbit/option/{checks,tests}`, `projects/funbit/spindraw/tests`, `projects/active/rising-stars/checks`).
  - `tests/`  — automated test cases (`.test.ts`)
  - `checks/` — online health checks (`.check.ts`)
  - `exports/` — data export scripts (`.export.ts`)
- APP scripts (Appium-based): `projects/app/<module>/` — 正式用例在 `projects/app/android-lite/`（含 `*.android.lite` 与 `*.ios.lita`）；`projects/app/core/` 只放 `_lib`、用例文档与兼容 check（**勿在 core/ 再放 `*.test.ts`，平台会扫成双份**）。platform/flavor/kind are encoded in the file name, so `checks`/`tests` subfolders are NOT used.
...
2. **Service layer** (`src/services/`) — env-aware business implementations. Construct with `'prod' | 'test'`; host auto-selected (`api.cinta.team` / `api.test.cinta.team`).
   ...
   - `DidibusService`（didibus-v202609，测试环境）：Active 接口（config/enter/detail/draw/marquee/gifts/records/debug）+ 模块接口（account/lucky-gift/lucky-mileage/bus/rank）、`sendGift`（gift_send 消息模拟）、`runCron`（北京时间精确到 cron 分钟）；MySQL 查询/造数（`setTicketBalance` 直写余额、`cleanUsers`/`cleanRounds`）、超发风控计数（`alarmIncome`/`alarmExpend`/`seedAlarm`/`cleanAlarm`，key `{biz}:common:alarm:income|expend`，Hash field=大区；2026-09-21 需求新增2：支出÷消耗 ≥102.5% 奖池不出 GIFT，011 用例覆盖）、Redis（`rankSeed` ZADD 直写榜单、`rankScore`/`rankTop`/`marqueeRaw`/`cleanRedis`；注意 010 收礼榜/贡献者数据必须走真实 `sendGift` 消息链路，禁直写）。注意：l-debug-timestamp 驱动有效期校验与业务时间，格式须为 `yyyy-MM-ddTHH:mm:ss`（北京）或带 `±HH:mm` 偏移（不支持毫秒/空格，解析失败会回退真实时间）；didibus 业务接口调用前需先 `POST v3/didibus-v202609/p/init {"round":1}` 初始化轮次（创建总榜/日榜/任务 mod_common_round，幂等，否则 enter 等写路径 200 但不落库）。v1.4.0 起抽奖为双 LuckydrawModule：`lucky-gift`（扣券，奖池 bus.normal/bus.flying）+ `lucky-mileage`（price=0，奖池 bus.mileage.normal/flying，每次 /draw 写两批 mod_luckydraw_record）。接口文档 v1.10.0：`/gifts` 返回 `{gifts, albumGifts}`（albumGifts=图鉴礼物 N-S-HEADBOX 4381 仅展示）；`/draw` 响应含 `awards`（=luckyGift.mergedAwards）、DrawResponse 含 `createTime`/`mergedAwards`；`/records`（lucky-gift 批次+mileage 合并+awards，游标分页）；`/debug`（按大区解析的起止时间）；模块 records 支持 `createTimes` 过滤。v1.5.0 起账户为 mod_account 体系：账户标识 `DIDIBUS-MILEAGE`（mod_account.name，id=901），余额/流水在 `mod_account_user`/`mod_account_user_record`（唯一键 (biz, trans_no) 幂等；送礼 transNo=send_/recv_+orderNo，/draw transNo 服务端生成，enter transNo=enter_{userId}_{yyyyMMdd}）；旧 active_coin/active_user_account 已废弃。
   - `SpinDrawService`（水果机 spin/draw + 图鉴 album，测试环境，litaroom/lita-basic）：接口鉴权不走 l-user-id，而是伪造 `AccessToken` 头（`makeAccessToken(userId)` —— @SessionVerifier 的 AccessTokenCheckLoginVerify 只做 AES 解密不查 Redis/不验过期，测试环境 AES key=`to_sigNon_eNckey`，与 funbit-api SessionConfig 非 PROD 一致；token 为 40 字节 RedisToken 二进制 AES/ECB/PKCS5+base64）。接口：`panel`（含 albumStar 可领取星级）/`albumPanel`/`doDraw`（返回 batchNo，须再 `confirmReceive` 才真正到手并累计图鉴进度）/`albumReceive`（领奖写 gift_award_queue 异步发放）/`openBlindPack`（litabasic 开袋）。存储：spin_draw_* 表在 **funbit** 库，gift_award_queue / user_blind_pack* 在 **lita_basic** 库（逻辑名 basic，直连须用物理名）；池内项以 awardType+awardId 复合 key 定位（礼物/道具 awardId 会撞号）；collect_date 按大区当地日期（getRankLocaleTimeZone 缺省 GMT+7:00）。注意：接口响应数字字段均序列化为字符串。造数/清理：`seedProgressToStar`（ON DUPLICATE GREATEST 保底）、`cleanAlbumData`/`cleanBlindPack`/`cleanRedis`（spinDraw:* 在房间 Redis 实例，清理 best-effort）。
...
3. **Business layer** (`src/base/`) — base classes for test cases and online checks.
   ...
   - `AppBaseClass` env: `env` from `SCRIPT_ENV` (`PROD` default / `TEST`); `scriptConfig` / `account()` 按环境选 `accounts.prod` 或 `accounts.test`（可回退 `default`）；TEST 缺省 OTP=`1234`。
