...
2. **Service layer** (`src/services/`) — env-aware business implementations. Construct with `'prod' | 'test'`; host auto-selected (`api.cinta.team` / `api.test.cinta.team`).
   ...
   - `DidibusService`（didibus-v202609，测试环境）：Active 接口（config/enter/detail/draw/marquee）+ 模块接口（account/lucky-gift/lucky-mileage/bus/rank）、`sendGift`（gift_send 消息模拟）、`runCron`（北京时间精确到 cron 分钟）；MySQL 查询/造数（`setTicketBalance` 直写余额、`cleanUsers`/`cleanRounds`）、Redis（`rankSeed` ZADD 直写榜单、`rankScore`/`rankTop`/`marqueeRaw`/`cleanRedis`；注意 010 收礼榜/贡献者数据必须走真实 `sendGift` 消息链路，禁直写）。注意：l-debug-timestamp 驱动有效期校验与业务时间，格式须为 `yyyy-MM-ddTHH:mm:ss`（北京）或带 `±HH:mm` 偏移（不支持毫秒/空格，解析失败会回退真实时间）；didibus 业务接口调用前需先 `POST v3/didibus-v202609/p/init {"round":1}` 初始化轮次（创建总榜/日榜/任务 mod_common_round，幂等，否则 enter 等写路径 200 但不落库）。v1.4.0 起抽奖为双 LuckydrawModule：`lucky-gift`（扣券，奖池 bus.normal/bus.flying）+ `lucky-mileage`（price=0，奖池 bus.mileage.normal/flying，每次 /draw 写两批 mod_luckydraw_record）。v1.5.0 起账户为 mod_account 体系：账户标识 `DIDIBUS-MILEAGE`（mod_account.name，id=901），余额/流水在 `mod_account_user`/`mod_account_user_record`（唯一键 (biz, trans_no) 幂等；送礼 transNo=send_/recv_+orderNo，/draw transNo 服务端生成，enter transNo=enter_{userId}_{yyyyMMdd}）；旧 active_coin/active_user_account 已废弃。
